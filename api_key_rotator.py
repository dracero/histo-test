...
Módulo standalone que proporciona rotación dinámica de API Keys de Google para langchain_google_genai.
"""
...
# ── Detección de errores de cuota o indisponibilidad de modelo ──
def _is_quota_error(exc: Exception) -> bool:
    err_str = str(exc).lower()
    return any(
        ind in err_str
        for ind in [
            "403", "429", "404", "401", "unauthenticated", "unauthorized",
            "not_found", "notfound",
...
def invoke_with_retry(llm, messages, *, rotator=None, max_retries=0, base_wait=2.0):
    """Invoca LLM con retry automático ante 403/429, rotando la API key dinámicamente."""
    if rotator is None:
        rotator = google_key_rotator
    if max_retries == 0:
